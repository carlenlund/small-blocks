...
function generateWorld(layer, zoom, x) {
  ...
  for (var i = 0; i < layer.nodes.length; ++i) {
    var noise = (noise2d(0.01 * (i - Math.floor(layerSize / 2) + chunkX * layerSize) * Math.pow(2, -zoom), 0) + 1) / 2;
    // var colorName = colorNames[Math.floor(noise * colorNames.length)];
    // layer.nodes[i].value = colorName;
    layer.nodes[i].value = 'rgb(' + Math.round(Math.floor(noise * 10) / 10 * 255) + ',' + Math.round(Math.floor(noise * 10) / 10 * 255) + ',' + Math.round(Math.floor(noise * 10) / 10 * 255) + ')';
  }
  layer.dirtyNegative = true;
  layer.dirtyPositive = true;
}
